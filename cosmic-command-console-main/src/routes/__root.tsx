...
import { AuthProvider, useAuth } from "@/lib/auth"; // ייבוא useAuth לצורך הבדיקה
...
// קומפוננטת עזר פנימית כדי להשתמש ב-useAuth בתוך ה-AuthProvider
function ContentWithAuth() {
  const { user } = useAuth(); // שליפת ה-user הנוכחי
  ...
  return (
    <>
      ...
      <Header />
      <Outlet />
      {/* הבוט יוצג אך ורק אם קיים משתמש מחובר ותפקידו מנהל מערכת (ROLE_ADMIN) */}
      {user && user.role === "ROLE_ADMIN" && <MissionAdvisor />} 
    </>
  );
}